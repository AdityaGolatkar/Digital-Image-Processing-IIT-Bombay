...
imgret = img; 
img = imgaussfilt(img,0.5);                                 %Gaussian filtering the image.
...
%=======================
    ...
    for i= winby2 +1: winby2 +sz(2)
        for j = winby2+1:winby2+sz(1)
            ...
            %w_m = (tanh(gmag(i-winby2:i+winby2,j-winby2:j+winby2)- m) + ones(win,win))/2;
            ...
            t_next(i,j,1) = num/normfact;
            num = sum(t_win2(:));
            %t_next(i,j,1) = num/(win*win);
            t_next(i,j,2) = num/normfact;
        end
    end
    t = t_next;
end
